const H = globalThis, k = H.ShadowRoot && (H.ShadyCSS === void 0 || H.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, L = Symbol(), B = /* @__PURE__ */ new WeakMap();
let Q = class {
  constructor(t, e, s) {
...
const it = (r) => new Q(typeof r == "string" ? r : r + "", void 0, L), rt = (r, ...t) => {
...
const { is: ot, defineProperty: ht, getOwnPropertyDescriptor: at, getOwnPropertyNames: lt, getOwnPropertySymbols: ct, getPrototypeOf: dt } = Object, M = globalThis, W = M.trustedTypes, pt = W ? W.emptyScript : "", ut = M.reactiveElementPolyfillSupport, b = (r, t) => r, R = { toAttribute(r, t) {
  switch (t) {
    case Boolean:
      r = r ? pt : null;
      break;
  ...
  return e;
} }, X = (r, t) => !ot(r, t), V = { attribute: !0, type: String, converter: R, reflect: !1, useDefault: !1, hasChanged: X };
...
A.elementStyles = [], A.shadowRootOptions = { mode: "open" }, A[b("elementProperties")] = /* @__PURE__ */ new Map(), A[b("finalized")] = /* @__PURE__ */ new Map(), ut?.({ ReactiveElement: A }), (M.reactiveElementVersions ??= []).push("2.1.1");
const D = globalThis, O = D.trustedTypes, q = O ? O.createPolicy("lit-html", { createHTML: (r) => r }) : void 0, Y = "$lit$", f = `lit$${Math.random().toFixed(9).slice(2)}$`, tt = "?" + f, $t = `<${tt}>`, g = document, w = () => g.createComment(""), C = (r) => r === null || typeof r != "object" && typeof r != "function", j = Array.isArray, ft = (r) => j(r) || typeof r?.[Symbol.iterator] == "function", N = `[ 	
\f\r]`, E = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, F = /-->/g, J = />/g, _ = RegExp(`>|${N}(?:([^\\s"'>=/]+)(${N}*=${N}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), K = /'/g, Z = /"/g, et = /^(?:script|style|textarea|title)$/i, _t = (r) => (t, ...e) => ({ _$litType$: r, strings: t, values: e }), mt = _t(1), y = Symbol.for("lit-noChange"), d = Symbol.for("lit-nothing"), G = /* @__PURE__ */ new WeakMap(), m = g.createTreeWalker(g, 129);
function st(r, t) {
...
const gt = (r, t) => {
  ...
  for (let l = 0; l < e; l++) {
    const h = r[l];
    let c, p, a = -1, u = 0;
    for (; u < h.length && (o.lastIndex = u, p = o.exec(h), p !== null); ) u = o.lastIndex, o === E ? p[1] === "!--" ? o = F : p[1] !== void 0 ? o = J : p[2] !== void 0 ? (et.test(p[2]) && (i = RegExp("</" + p[2], "g")), o = _) : p[3] !== void 0 && (o = _) : o === _ ? p[0] === ">" ? (o = i ?? E, a = -1) : p[1] === void 0 ? a = -2 : (a = o.lastIndex - p[2].length, c = p[1], o = p[3] === void 0 ? _ : p[3] === '"' ? Z : K) : o === Z || o === K ? o = _ : o === F || o === J ? o = E : (o = _, i = void 0);
    const $ = o === _ && r[l + 1].startsWith("/>") ? " " : "";
    n += o === E ? h + $t : a >= 0 ? (s.push(c), h.slice(0, a) + Y + h.slice(a) + f + $) : h + f + (a === -2 ? l : $);
  }
    ...
    for (; (i = m.nextNode()) !== null && h.length < l; ) {
      if (i.nodeType === 1) {
        if (i.hasAttributes()) for (const a of i.getAttributeNames()) if (a.endsWith(Y)) {
          const u = p[o++], $ = i.getAttribute(a).split(f), U = /([.?@])?(.*)/.exec(u);
          h.push({ type: 1, index: n, name: U[2], strings: $, ctor: U[1] === "." ? yt : U[1] === "?" ? vt : U[1] === "@" ? Et : T }), i.removeAttribute(a);
        ...
        if (et.test(i.tagName)) {
          ...
          if (u > 0) {
            ...
            for (let $ = 0; $ < u; $++) i.append(a[$], w()), m.nextNode(), h.push({ type: 2, index: ++n });
            i.append(a[u], w());
          }
        }
...
St?.(P, x), (D.litHtmlVersions ??= []).push("3.3.1");
const wt = (r, t, e) => {
...
class Pt extends S {
  ...
  handleLogin(t) {
    t.preventDefault();
    const e = this.shadowRoot.getElementById("email").value, s = this.shadowRoot.getElementById("password").value;
    e && s ? this.message = `Welcome back, ${e}!` : this.message = "Please enter email and password.";
  }
  render() {
    return mt`
            <h3>Login</h3>
...
customElements.get("feature-login") || customElements.define("feature-login", Pt);
function Ht(r, t = {}) {
  if (!r) return;
  const e = document.createElement("feature-login");
  Object.assign(e, t), r.innerHTML = "", r.appendChild(e);
}
function Ot(r) {
  r.innerHTML = "";
}
